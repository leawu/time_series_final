...
yearly_trips <- mutate(df, Year = year(Trip.Start.Timestamp))
yearly_trips <- summarise(yearly_trips, Total_Trips = sum(count), .by = Year)  
print(yearly_trips)
...
```{r}
daily_trips <- mutate(df, Date = as.Date(Trip.Start.Timestamp))
daily_trips <- summarise(daily_trips, trips_count = sum(count), .by = Date) 
